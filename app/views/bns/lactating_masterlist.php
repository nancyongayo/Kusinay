...
$total  = count($records);
function bmiColor(string $s): string {
    return match($s) {
        'Underweight' => '#e67e22',
        'Normal'      => '#27ae60',
        'Overweight'  => '#2980b9',
...
<div class="screen-toolbar no-print" style="margin-bottom:1.5rem">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2">
            ...
            <div>
                <div class="fw-bold" style="color:var(--kn-dark)">Lactating Mothers Masterlist</div>
                <div style="font-size:.78rem;color:var(--kn-muted)">Quarterly BMI Monitoring Sheet</div>
            </div>
        </div>
        <div class="d-flex gap-2 align-items-center">
            <form method="GET" action="index.php" class="d-flex gap-2 align-items-center flex-wrap">
                <input type="hidden" name="action" value="lactatingMasterlist">
...
<div class="form-wrapper">
    ...
    <div class="form-title-bar">
        <div style="font-weight:700;font-size:.92rem;color:var(--kn-dark)">
            Masterlist and Monitoring Sheet of <span style="color:var(--kn-green)">Lactating Mothers</span>
        </div>
        <div class="d-flex align-items-center gap-2">
    ...
    <div style="padding:0 1.5rem 1.5rem;overflow-x:auto">
        ...
        <div class="empty-state">
            ...
            <div class="fw-semibold" style="color:var(--kn-dark)">No lactating mothers assessed for <?= $qLabel ?> <?= $year ?></div>
        </div>
        <?php else: ?>
        <table class="ml-table" style="margin-top:1rem">
            <thead>
                <tr>
                    ...
                    <th style="width:80px">BMI Status</th>
                    <th style="width:80px">Breastfeeding</th>
            ...
            <tr>
                ...
                <td><?= $r['age_in_years'] !== null ? (int)$r['age_in_years'] . ' yrs' : '—' ?></td>
                ...
                <td>
                    <?php if ($r['bmi_status']): ?>
                    <span class="ns-chip" style="background:<?= bmiColor($r['bmi_status']) ?>">
                        <?= $r['bmi_status'] ?>
                    </span>
                    <?php else: ?>—<?php endif; ?>
                </td>
                <td style="font-size:.72rem"><?= htmlspecialchars($r['breastfeeding_status'] ?? '—') ?></td>
                <td><?= htmlspecialchars($r['contact'] ?? '—') ?></td>
                <td><?= $r['assessment_date'] ? date('M j, Y', strtotime($r['assessment_date'])) : '<span style="color:#e67e22;font-size:.7rem">Not yet assessed</span>' ?></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
<div class="ml-paper">
    ...
    <div style="text-align:center;font-weight:800;font-size:10pt;margin-bottom:4px">
        MASTERLIST AND MONITORING SHEET OF LACTATING MOTHERS — <?= strtoupper($qLabel) ?> CY <?= $year ?>
    </div>
    <table class="ml-ptable">
        <thead>
            <tr>
                ...
                <th style="width:70px">Breastfeeding Status</th>
        ...
        <tbody>
        <?php if (empty($records)): ?>
        <tr><td colspan="12" style="text-align:center;padding:8px">No records.</td></tr>
        <?php else: ?>
        <?php foreach ($records as $i => $r): ?>
        <tr>
            <td><?= $i+1 ?></td>
            ...
            <td><?= $r['age_in_years'] !== null ? (int)$r['age_in_years'] : '' ?></td>
            <td class="tl"><?= htmlspecialchars($r['purok'] ?? '') ?></td>
            <td><?= $r['height_cm'] ?? '' ?></td>
            <td><?= $r['weight_kg'] ?? '' ?></td>
            <td><?= $r['bmi'] ?? '' ?></td>
            <td><?= $r['bmi_status'] ?? '' ?></td>
            <td><?= htmlspecialchars($r['breastfeeding_status'] ?? '') ?></td>
            <td><?= htmlspecialchars($r['contact'] ?? '') ?></td>
            <td><?= $r['assessment_date'] ? date('m/d/y', strtotime($r['assessment_date'])) : '' ?></td>
        </tr>
        <?php endforeach; ?>
        <?php for ($b = count($records); $b < 15; $b++): ?>
        <tr><td><?= $b+1 ?></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
        <?php endfor; ?>
        <?php endif; ?>
        </tbody>
    </table>
